""" This file defines policy optimization for a tensorflow_code policy. """
import copy
import logging
import tempfile
...
class PolicyOptTf(PolicyOpt):
    ...
    def update(self, obs, tgt_mu, tgt_prc, tgt_wt):
        ...
        Returns:
            A tensorflow_code object with updated weights.
        """
        print(" ================================ update policy!!! ===============================")
        N, T = obs.shape[:2]
        ...
        if self._hyperparams['fc_only_iterations'] > 0:
            ...
            for i in range(self._hyperparams['fc_only_iterations'] ):
                ...
                if (i+1) % 500 == 0:
                    LOGGER.info('tensorflow_code iteration %d, average loss %f',
        ...
        # Keep track of tensorflow_code iterations for loading solver states.
